...
def get_twap(df, col_name, field_name):
    ...
    ps1 = df.groupby(pd.Grouper(freq=freq)).apply(wavg, field_name, "ts_lag")
...
for sheet in worksheets:
    ...
    for filename in filenames:
        ...
        #twap_dfs.append(get_twap(data_df, filename, 'full_price'))
        twap_dfs.append(get_twap(data_df, filename, 'price'))
        #twap_dfs.append(get_twap(data_df, filename, 'net_price'))
    '''dfOutput = pd.concat([twap_dfs[0]['ts_date'],
                          ...
                          axis=1)
'''
    #twap_dfs[0].to_csv(path_proj + sheet + '_' + freq + "_twap_full_price.csv")
    twap_dfs[0].to_csv(path_proj + sheet + '_' + freq + "_twap_price.csv")
    #twap_dfs[1].to_csv(path_proj + sheet + '_' + freq + "_twap_net_price.csv")
    ...
    #dfOutput['diff'] = dfOutput['220210'] - dfOutput['220215']
